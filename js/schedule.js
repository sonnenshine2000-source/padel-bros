...
async function loadMoveControls(assignments){
  const playerSelect=$('movePlayer'); if(!playerSelect)return;
  playerSelect.innerHTML='<option value="">Spieler auswählen …</option>'+(assignments.filter(x=>x.player_id).map(x=>`<option value="${x.player_id}">${escapeHtml(x.players?.name||'Spieler')} · ${x.court==='court5'?'Court 5':'Court 1'} · Platz ${x.position}</option>`).join(''));
}

async function movePlayer(){
  if(!state.currentPlayer?.is_admin||!state.matchDay?.id)return;
  const playerId=Number($('movePlayer')?.value), targetCourt=$('moveCourt')?.value, targetPosition=Number($('movePosition')?.value);
  if(!playerId||!targetCourt||!targetPosition){msg($('moveStatus'),'Bitte Spieler, Court und Platz auswählen.');return;}
  const q=await supabase.from('assignments').select('id,player_id,court,position').eq('match_day_id',state.matchDay.id).order('position');
  if(q.error){msg($('moveStatus'),q.error.message);return;}
  const rows=q.data||[]; const source=rows.find(x=>x.player_id===playerId); if(!source){msg($('moveStatus'),'Spieler ist im aktuellen Spielplan nicht zugeordnet.');return;}
  if(source.court===targetCourt&&Number(source.position)===targetPosition){msg($('moveStatus'),'Der Spieler steht bereits auf diesem Platz.');return;}
  const target=rows.find(x=>x.court===targetCourt&&Number(x.position)===targetPosition);
  if(target&&target.id===source.id)return;
  if(!confirm(`${source.court==='court5'?'Court 5':'Court 1'} Platz ${source.position} → ${targetCourt==='court5'?'Court 5':'Court 1'} Platz ${targetPosition}${target?.player_id?'\n\nDie beiden Spieler werden getauscht.':''}`))return;
  if(target?.player_id){
    ...
    const b=await supabase.from('assignments').update({court:source.court,position:source.position,manually_changed:true}).eq('id',target.id);
    if(b.error){msg($('moveStatus'),'Der Tausch konnte nicht vollständig gespeichert werden: '+b.error.message);return;}
  }else{
    const a=await supabase.from('assignments').update({court:targetCourt,position:targetPosition,manually_changed:true}).eq('id',source.id);
    if(a.error){msg($('moveStatus'),a.error.message);return;}
  }

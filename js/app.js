...
export async function loadAll() {
  ...
  if (!data) { $('heroDate').textContent = niceDate(date); $('pollStatus').textContent = 'Für diesen Dienstag wurde noch kein Spieltag angelegt.'; updateAdminVisibility(); await loadCancelledCourts(); return; }
  state.matchDay = data;
  $('heroDate').textContent = niceDate(data.match_date);
  await Promise.all([loadPoll(), loadAssignments(), loadMatches(), loadPayments(), loadCancelledCourts()]);

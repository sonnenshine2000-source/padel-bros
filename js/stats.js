...
function validSet(a,b){
 if(!Number.isFinite(a)||!Number.isFinite(b)||a<0||b<0||a>7||b>7||a===b)return false;
 const hi=Math.max(a,b),lo=Math.min(a,b);
 return (hi===6&&lo<=4)||(hi===7&&(lo===5||lo===6));
}

function resultSets(r){
 return [[r.set1_home,r.set1_away],[r.set2_home,r.set2_away],[r.set3_home,r.set3_away]]
  .filter(([a,b])=>a!=null&&b!=null&&!(Number(a)===0&&Number(b)===0))
  .map(([a,b])=>[Number(a),Number(b)])
  .filter(([a,b])=>validSet(a,b));
}
...
export async function loadStats(){
 ...
 for(const d of q.data||[]){
  const by={court5:[],court1:[]};
  for(const a of d.assignments||[])if(by[a.court])by[a.court].push(a);
  for(const r of d.match_results||[]){
   const sets=resultSets(r);if(sets.length<2)continue;
   let aSets=0,bSets=0,aPts=0,bPts=0;
   for(const [a,b] of sets){aPts+=a;bPts+=b;if(a>b)aSets++;else bSets++;}
   if(aSets===bSets||Math.max(aSets,bSets)!==2)continue;
   const fallback=(by[r.court]||[]).sort((a,b)=>(a.position||0)-(b.position||0));
   const teamAIds=[r.team1_player1,r.team1_player2].map(Number).filter(Boolean);
   const teamBIds=[r.team2_player1,r.team2_player2].map(Number).filter(Boolean);
   const teamA=teamAIds.length===2?teamAIds:fallback.slice(0,2).map(x=>Number(x.player_id));
   const teamB=teamBIds.length===2?teamBIds:fallback.slice(2,4).map(x=>Number(x.player_id));
   if(teamA.length!==2||teamB.length!==2||new Set([...teamA,...teamB]).size!==4)continue;
   const namesById=new Map((d.assignments||[]).map(a=>[Number(a.player_id),a.players?.name||'Spieler']));
   ...
   addPair(teamA,teamA.map(id=>namesById.get(id)||'Spieler'),aWin);
   addPair(teamB,teamB.map(id=>namesById.get(id)||'Spieler'),!aWin);
  }
 }

 const rows=[...players.values()].sort((a,b)=>(b.wins/(b.matches||1))-(a.wins/(a.matches||1))||b.wins-a.wins||b.matches-a.matches||a.name.localeCompare(b.name));
 ...
 const bestPairs=[...pairs.values()].filter(x=>x.matches>=2).map(x=>({...x,pct:Math.round(x.wins/x.matches*100)})).sort((a,b)=>b.pct-a.pct||b.wins-a.wins||b.matches-a.matches).slice(0,8);
 ...
 const most=rows.reduce((a,b)=>!a||b.matches>a.matches?b:a,null);
 const mostWins=rows.reduce((a,b)=>!a||b.wins>a.wins?b:a,null);
 const streak=rows.reduce((a,b)=>!a||b.bestStreak>a.bestStreak?b:a,null);
 const best=qualified.length?[...qualified].sort((a,b)=>b.wins-a.wins||b.wins/(b.matches||1)-a.wins/(a.matches||1))[0]:null;
 el.innerHTML=`<div class="stat-hero"><div><small>AKTUELLE STATISTIK</small><h2>Wer ist in Form? 🔥</h2></div>${best?`<div class="stat-highlight">🏆 <b>${escapeHtml(best.name)}</b><span>${pct(best)}% Siegesquote</span></div>`:''}</div><div class="stats-grid"><section class="stat-box"><h3>🏆 Rangliste</h3><div class="sub">Offizielle Rangliste ab 5 Matches.</div>${ranking||'<div class="sub">Noch keine Spieler mit 5 Matches.</div>'}</section><section class="stat-box"><h3>📊 Alle Spieler</h3>${table||'<div class="sub">Noch keine Daten.</div>'}</section><section class="stat-box"><h3>🤝 Beste Paarungen</h3>${pairHtml}</section><section class="stat-box"><h3>🔥 Rekorde</h3><div class="record"><b>🎯 Meiste Matches</b><span>${most?escapeHtml(most.name):'—'} · ${most?.matches||0}</span></div><div class="record"><b>🏆 Meiste Siege</b><span>${mostWins?escapeHtml(mostWins.name):'—'} · ${mostWins?.wins||0}</span></div><div class="record"><b>🔥 Längste Siegesserie</b><span>${streak?escapeHtml(streak.name):'—'} · ${streak?.bestStreak||0}</span></div></section></div>`;
}

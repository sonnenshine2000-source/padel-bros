...
export async function loadStats(){
 ...
 const q=await supabase.from('match_days').select('match_date,assignments(court,position,player_id,players:player_id(id,name)),match_results(court,set1_home,set1_away,set2_home,set2_away,set3_home,set3_away,team1_player1,team1_player2,team2_player1,team2_player2,team1_player1_side,team1_player2_side,team2_player1_side,team2_player2_side)').order('match_date',{ascending:true});
 if(q.error){el.innerHTML=`<div class="status err">${escapeHtml(q.error.message)}</div>`;return;}
 const players=new Map(),pairs=new Map();
 const addPlayer=(id,name)=>{if(!id)return null;if(!players.has(id))players.set(id,{id,name:name||'Spieler',matches:0,wins:0,losses:0,setsW:0,setsL:0,pointsW:0,pointsL:0,streak:0,bestStreak:0,court5:0,court1:0,left:0,right:0});return players.get(id)};
 ...
 for(const d of q.data||[]){
  ...
  for(const r of d.match_results||[]){
   ...
   const sideById=new Map([[Number(r.team1_player1),r.team1_player1_side],[Number(r.team1_player2),r.team1_player2_side],[Number(r.team2_player1),r.team2_player1_side],[Number(r.team2_player2),r.team2_player2_side]]);
   const update=(ids,win)=>ids.forEach(id=>{const p=addPlayer(id,namesById.get(id));p.matches++;p.court5+=r.court==='court5'?1:0;p.court1+=r.court==='court1'?1:0;const side=sideById.get(Number(id));if(side==='links')p.left++;if(side==='rechts')p.right++;if(win){p.wins++;p.streak++;p.bestStreak=Math.max(p.bestStreak,p.streak);p.setsW+=aWin?aSets:bSets;p.setsL+=aWin?bSets:aSets;p.pointsW+=aWin?aPts:bPts;p.pointsL+=aWin?bPts:aPts;}else{p.losses++;p.streak=0;p.setsW+=aWin?bSets:aSets;p.setsL+=aWin?aSets:bSets;p.pointsW+=aWin?bPts:aPts;p.pointsL+=aWin?aPts:bPts;}});
 ...
 const table=rows.map(p=>`<div class="stat-row all-player-row"><b>${escapeHtml(p.name)}</b><span class="stat-matches">${p.matches} Spiele</span><span class="stat-record">${p.wins}–${p.losses}</span><strong class="stat-pct">${pct(p)}%</strong><span class="stat-sets">${p.setsW}:${p.setsL} Sätze</span><span class="stat-sides">⬅️ ${p.left} · ➡️ ${p.right}</span></div>`).join('');
 ...
 const sideTotals=rows.reduce((a,p)=>({left:a.left+p.left,right:a.right+p.right}),{left:0,right:0});
 el.innerHTML=`<div class="stat-hero"><div><small>AKTUELLE STATISTIK</small><h2>Wer ist in Form? 🔥</h2></div>${best?`<div class="stat-highlight">🏆 <b>${escapeHtml(best.name)}</b><span>${pct(best)}% Siegesquote</span></div>`:''}</div><div class="stats-grid"><section class="stat-box"><h3>🏆 Rangliste</h3><div class="sub">Offizielle Rangliste ab 5 Matches.</div>${ranking||'<div class="sub">Noch keine Spieler mit 5 Matches.</div>'}</section><section class="stat-box"><h3>📊 Alle Spieler</h3><div class="sub">⬅️ Links · ➡️ Rechts</div>${table||'<div class="sub">Noch keine Daten.</div>'}</section><section class="stat-box"><h3>🤝 Beste Paarungen</h3>${[...pairs.values()].filter(x=>x.matches>=2).map(x=>({...x,pct:Math.round(x.wins/x.matches*100)})).sort((a,b)=>b.pct-a.pct||b.wins-a.wins||b.matches-a.matches).slice(0,8).map(x=>`<div class="stat-row"><b>${escapeHtml(x.names.join(' + '))}</b><span>${x.matches} Matches</span><strong>${x.wins} Siege · ${x.pct}%</strong></div>`).join('')||'<div class="sub">Noch nicht genug gemeinsame Matches.</div>'}</section><section class="stat-box"><h3>🔥 Rekorde</h3><div class="record"><b>🎯 Meiste Matches</b><span>${maxMatches?`${names(mostPlayers)} · ${maxMatches}`:'—'}</span></div><div class="record"><b>🏆 Meiste Siege</b><span>${maxWins?`${names(mostWinPlayers)} · ${maxWins}`:'—'}</span></div><div class="record"><b>🔥 Längste Siegesserie</b><span>${maxStreak?`${names(streakPlayers)} · ${maxStreak}`:'—'}</span></div><div class="record"><b>⬅️➡️ Gespielte Seiten</b><span>${sideTotals.left}× Links · ${sideTotals.right}× Rechts</span></div></section></div>`;
}

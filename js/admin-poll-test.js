...
export function bindAdminPollTest(){
  ...
  if(!card||document.querySelector('#adminPollTest')) return;
...
async function loadPlayers(){
  ...
  if(!sel||!state.currentPlayer?.is_admin) return;
  const q=await supabase.from('players').select('id,name,active').eq('active',true).order('name');
  if(q.error){document.querySelector('#testPollStatus').textContent=q.error.message;return;}
  sel.innerHTML='<option value="">Spieler auswählen …</option>'+(q.data||[]).map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');
}

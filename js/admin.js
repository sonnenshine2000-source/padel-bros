...
export async function loadPlayerAdmin() {
  ...
  if (q.error) {
    $('playerAdminList').innerHTML =
      '<div class="status err">' + escapeHtml(q.error.message) + '</div>';
    return;
  }
        ...
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:center">
          <div>
            ...
            <div class="sub" style="margin:4px 0 0">
              ...
              ·
              ${p.auth_user_id ? 'PIN eingerichtet' : 'Noch keine PIN'}
            </div>
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap">
            <button class="smallAdmin" data-action="pin" data-id="${p.id}">🔑 PIN</button>
            <button class="smallAdmin" data-action="active" data-id="${p.id}">
              ${p.active === false ? 'Freigeben' : 'Sperren'}
            </button>
            <button class="smallAdmin" data-action="stamm" data-id="${p.id}">
...
export function bindAdmin() {
  ...
  msg(
    ...
    if (action === 'pin') {
      const pin = prompt('Neue 6-stellige PIN für diesen Spieler:');
      if (!pin) return;
      ...
      if (!res.ok) {
        msg(
          $('adminStatus'),
          body.error || 'PIN konnte nicht gesetzt werden.'
        );
      } else {
        msg($('adminStatus'), 'PIN wurde erfolgreich gesetzt.', true);
    ...
    if (action === 'active') {
      ...
      if (!res.ok) {
        msg(
          $('adminStatus'),
          body.error || 'Status konnte nicht geändert werden.'
        );
      } else {
        msg($('adminStatus'), 'Spielerstatus geändert.', true);
        await loadPlayerAdmin();
      }
      return;
    }
    ...
    if (action === 'stamm') {
      ...
      const r = await supabase
        .from('players')
        .update({
          is_stammspieler: !row.data.is_stammspieler
        })
        .eq('id', id);

      if (r.error) {
        msg($('adminStatus'), r.error.message);
      } else {
        await loadPlayerAdmin();
      }
    }

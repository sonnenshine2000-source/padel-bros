...
export async function loadPayments(){
  ...
  const q=await supabase.from('payments').select('id,match_day_id,amount,paid,paid_at,paid_by,substitute_id,stammspieler_id,players_sub:substitute_id(name),players_main:stammspieler_id(name,paypal_email),match_days:match_day_id(match_date)').eq('match_day_id',day.id).order('id',{ascending:false});

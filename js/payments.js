...
export async function loadPayments(){
  ...
  const q=await supabase.from('payments').select('id,match_day_id,amount,paid,paid_at,paid_by,substitute_id,stammspieler_id,players_sub:substitute_id(name),players_main:stammspieler_id(name,paypal_email,paypal_me_url),match_days:match_day_id(match_date)').eq('match_day_id',day.id).order('id',{ascending:false});
  ...
  $('payments').innerHTML='<div class="list">'+q.data.map(p=>{
    ...
    const paypal=p.players_main?.paypal_email||''; const direct=paypalLink(p.players_main?.paypal_me_url,p.amount);
